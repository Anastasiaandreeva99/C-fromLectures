// Exercise 9.3 Solution: LINQInvoiceArray.cs
// Use LINQ to extract data from an array of Invoice objects.
using System;
using System.Linq;
    ...
    public class LINQInvoiceArray
    {
        public static void Main(string[] args)
        {
         ...
         new Invoice( 3, "Wrench", 34, 7.5M ) }; // end initializer list
            ...
            // after C# 7 return a Tuple
            IEnumerable<(string, decimal)> descriptionAndTotalTuple =
               from item in invoices
               let total = item.Quantity * item.Price
               orderby total
               select ( item.PartDescription,  total );

            var descriptionAndTotalTup = invoices.Select(item=> (Desc:item.PartDescription, Num:item.PartNumber) );
            foreach  (var  item in descriptionAndTotalTup)
            {
            ...
            var groupedGreaterThan12 =
                ...
                select new { K = de.Key, Detail = de };

            foreach (var item in groupedGreaterThan12)
            {
                Console.WriteLine("Category: {0}", (item.K) ? "Price below $12" : "Price above $12");
            ...
            // return a Tupple after C#7
            IEnumerable<( int, IGrouping<int,Invoice>)> groupedGreaterTupple =
                from item in invoices
                orderby item.Price
                group item by FindInterval(item.Price) into de
                select (de.Key, de );
